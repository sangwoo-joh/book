	...
	function displaySearchResults(results, query) {
		...
		if (results.length) {
			...
			results.forEach(function (result) {
				...
				if (("" == "true") && ("true" == "true") && (!searchVersions.includes(item.version))) {
				    return
				}
					...
					if (searchVersions.length == 1 ||  "" == "false"){
						versionBadge = ""
					...
					resultsHTML += "<li><h4><a href='/book" + item.url.trim() + "'>" + titlePreview + "</a></h4><p>" + versionBadge +"<small>" + contentPreview + "</small></p></li>";
				}
			});
